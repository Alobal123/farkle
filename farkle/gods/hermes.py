...
- Level 3: 6 more commerce goals (12 total)
...
class Hermes(God):
    ...
    def __init__(self, game=None):
        ...
        self.goals_completed = 0  # Track commerce goals completed
    
    def on_activate(self, game):  # type: ignore[override]
        """Called when Hermes is added to worshipped gods."""
        super().on_activate(game)
    
    def on_deactivate(self, game):  # type: ignore[override]
        """Called when Hermes is removed from worshipped gods."""
        super().on_deactivate(game)
    
    def on_event(self, event: GameEvent):  # type: ignore[override]
        """Listen for Commerce goal completions and reward events."""
        ...
        # Level 3: Double rewards for commerce goals
        elif self.level >= 3:
    ...
    def _level_up_from_goal(self):
        """Level up when a commerce goal is completed."""
        ...
        if self.goals_completed >= goals_needed:
            ...
            # Add level 1 bonus: +20% to commerce goals
            if self.level == 1:
    ...
    def _add_level_1_bonus(self):
        """Add +20% scoring bonus to commerce goals."""
        try:
            # Create a conditional modifier that only applies to commerce category goals
            category_predicate = lambda ctx: getattr(getattr(ctx, 'goal', None), 'category', '') == 'commerce'
            modifier = ConditionalScoreModifier(
                GlobalPartsMultiplier(1.2, priority=58, description="Hermes Level 1"),
            ...
            self.emit(self.game, GameEventType.SCORE_MODIFIER_ADDED, {
                ...
                "description": "+20% to commerce goals",
                "data": {"category": "commerce", "mult": 1.2}
            })
    ...
    def _add_level_2_ability(self):
        """Add Sanctify ability to change goal categories to commerce."""
        try:
    ...
    def _maybe_double_gold_reward(self, event: GameEvent):
        """Double gold rewards from commerce goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
        
        # Only double goal rewards (not temple income or god bonuses)
        if source == "goal_reward" and category == "commerce" and amount > 0:
            try:
    ...
    def _maybe_double_income_reward(self, event: GameEvent):
        """Double income rewards from commerce goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
    ...
    def _maybe_double_faith_reward(self, event: GameEvent):
        """Double faith rewards from commerce goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
    ...
    def _maybe_double_blessing_reward(self, event: GameEvent):
        """Double blessing rewards from commerce goals at level 3 (extends duration)."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        blessing_type = event.get("blessing_type", "")
        
        # Only double goal blessings (not god bonuses to avoid infinite loop)
        if source == "goal_reward" and category == "commerce" and blessing_type:
            try:
