...
- Level 3: 6 more nature goals (12 total)

Favor Progression:
- Level 1: +20% scoring bonus to nature goals
- Level 2: Sanctify ability (1 use) - change a goal's category to nature
"""
...
class Demeter(God):
    ...
    def __init__(self, game=None):
        super().__init__(name="Demeter", game=game)
        self.level = 0  # Start at level 0
        self.goals_completed = 0  # Track nature goals completed
    
    def on_activate(self, game):  # type: ignore[override]
        """Called when Demeter is added to worshipped gods."""
        super().on_activate(game)
    
    def on_deactivate(self, game):  # type: ignore[override]
        """Called when Demeter is removed from worshipped gods."""
        super().on_deactivate(game)
    
    def on_event(self, event: GameEvent):  # type: ignore[override]
        """Listen for Nature goal completions and reward events."""
        ...
        # Level 3: Double rewards for nature goals
        elif self.level >= 3:
    ...
    def _level_up_from_goal(self):
        """Level up when a nature goal is completed."""
        ...
        if self.goals_completed >= goals_needed:
            ...
            # Add level 1 bonus: +20% to nature goals
            if self.level == 1:
    ...
    def _add_level_1_bonus(self):
        """Add +20% scoring bonus to nature goals."""
        try:
            # Create a conditional modifier that only applies to nature category goals
            category_predicate = lambda ctx: getattr(getattr(ctx, 'goal', None), 'category', '') == 'nature'
            modifier = ConditionalScoreModifier(
                GlobalPartsMultiplier(1.2, priority=58, description="Demeter Level 1"),
            ...
            self.emit(self.game, GameEventType.SCORE_MODIFIER_ADDED, {
                ...
                "description": "+20% to nature goals",
                "data": {"category": "nature", "mult": 1.2}
            })
    ...
    def _add_level_2_ability(self):
        """Add Sanctify ability to change goal categories to nature."""
        try:
    ...
    def _maybe_double_gold_reward(self, event: GameEvent):
        """Double gold rewards from nature goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
        
        # Only double goal rewards (not temple income or god bonuses)
        if source == "goal_reward" and category == "nature" and amount > 0:
            try:
    ...
    def _maybe_double_income_reward(self, event: GameEvent):
        """Double income rewards from nature goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
    ...
    def _maybe_double_faith_reward(self, event: GameEvent):
        """Double faith rewards from nature goals at level 3."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        amount = event.get("amount", 0)
    ...
    def _maybe_double_blessing_reward(self, event: GameEvent):
        """Double blessing rewards from nature goals at level 3 (extends duration)."""
        source = event.get("source", "")
        category = event.get("goal_category", "")
        blessing_type = event.get("blessing_type", "")
        
        # Only double goal blessings (not god bonuses to avoid infinite loop)
        if source == "goal_reward" and category == "nature" and blessing_type:
            try:
